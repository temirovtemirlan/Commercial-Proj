...
const SectionHeader: FC = () => {
  ...
  const [isFirstHeaderVisible, setIsFirstHeaderVisible] = useState(false);
  ...
  const handleScroll = () => {
    ...
    requestRef.current = requestAnimationFrame(() => {
      // Добавляем небольшой интервал перед изменением состояния
      if (
        secondHeaderRef.current &&
        window.scrollY > secondHeaderRef.current?.offsetTop + 5
      ) {
        // Добавляем задержку для начала анимации, чтобы она не запускалась сразу
        if (!isFirstHeaderVisible) {
          setTimeout(() => setIsFirstHeaderVisible(true), 50);
        }
      } else {
        setIsFirstHeaderVisible(false);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      cancelAnimationFrame(requestRef.current); // Очищаем анимацию при размонтировании
    };
  }, [isFirstHeaderVisible]);
  ...
  return (
    <div>
        ...
        <AnimatePresence>
          {isFirstHeaderVisible && (
            <motion.div
              key="firstHeader"
              initial={{ opacity: 0 }}
              animate={{ opacity: isFirstHeaderVisible ? 1 : 0 }}
              exit={{ opacity: 0, transition: { duration: 0 } }}
              transition={{ duration: 0.7 }}
              className="fixed top-0 flex items-center w-full bg-white md:px-[100px] px-5 z-50 box-border py-2.5 h-[60px]"
            >
      ...
      <Container className="py-[70px] -mt-[60px]">
      ...
      <motion.div ref={secondHeaderRef}>
